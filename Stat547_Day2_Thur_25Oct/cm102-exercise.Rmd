...
fox2 <- str_subset(sentences, pattern = "fox")
...
```{r}
str_split(fox, pattern = "") # no space
str_split(fox, pattern = " ") # one space
str_split(fox, pattern = "  ") # two spaces
str_split(fox, pattern = "   ") # three spaces
```
...
```{r}
str_view_all_match(str_to_lower(countries), pattern = "[^aeiou]")
str_view_all_match(str_to_lower(countries), pattern = "[^aeiou][^aeiou][^aeiou]")
str_view_all_match(str_to_lower(countries), pattern = "[^aeiou ][^aeiou ][^aeiou ]")
```
...
```{r}
c("bear", "beer", "bar") %>% 
    str_view_all_match(pattern = "bee|ar")  # bee or ar
```
...
```{r}
str_view_all_match(countries, "e{2}|ae+|ea+|a{2}")
```


```{r}
str_view_all_match(countries, "(a|e)(a|e)")
str_view_all_match(countries, "(a|e){2}")
```
...
```{r}
str_view_all_match(countries, pattern="e{2}|")
```
...
```{r}
str_view_all_match(countries, pattern = "^Ca") # ^ not in square bracket mean 'starts'
```

Countries without a vowel? The word should start with a non-vowel, continue as a non-vowel, and end:

```{r}
str_view_all_match(countries, "^[^aeiouAEIOU]*$") # ^  in square bracket mean 'does not contain'
```
...
```{r}
str_view_all(c("bananas"), "(.)(.)\\1\\2.*\\1\\2")  # (.)(.) anything follows by anything
# \\1\\2 group one is the first 'a' and group 2 'n'
# .* followed by anything
# \\1\\2  find group 1 followed by group 2
```
