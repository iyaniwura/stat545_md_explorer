...
```
```{r}
pmap(list(a, b, c), ~ ..1 * ..2 * ..3)  # produces a list
```
```{r}
pmap_dbl(list(a, b, c), ~ ..1 * ..2 * ..3)  # produces a double vectors
```
...
```{r}
map2(name, commute, function(x, y) str_c(x, " takes ", y, " minutes to get to work."))
```
...
```{r}
(alias_list <- got_alias[1]) # data frame
